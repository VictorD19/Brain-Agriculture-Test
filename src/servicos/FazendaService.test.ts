...
describe("Testes de Validação de Dados para Criação de Fazenda", () => {
  ...
  it("Deve passar quando todos os dados forem válidos", () => {
    ...
    expect(fazendaService.ValidarDadosFazenda(fazendaValida)).toBe("");
  });

  it("Deve retornar erro quando o nome da fazenda estiver faltando", () => {
    ...
    expect(fazendaService.ValidarDadosFazenda(fazendaSemNome) ).toBe(
      "Precisa informar um nome para sua fazenda"
    );
  });

  it("Deve retornar erro quando o estado estiver faltando", () => {
    ...
    expect(fazendaService.ValidarDadosFazenda(fazendaSemEstado)).toBe(
      "Precisa informar o estado onde se encontra sua fazenda"
    );
  });

  it("Deve retornar erro quando a cidade estiver faltando", () => {
    ...
    expect(fazendaService.ValidarDadosFazenda(fazendaSemCidade)).toBe(
      "Precisa informar a cidade onde se encontra sua fazenda"
    );
  });

  it("Deve retornar erro quando a área agricultável for zero", () => {
    ...
    ).toBe("Precisa informar a área agricultável");
  });

  it("Deve retornar erro quando a área total for zero", () => {
    ...
    expect(fazendaService.ValidarDadosFazenda(fazendaAreaTotalZero)).toBe(
      "Precisa informar a área total da sua fazenda"
    );
  });

  it("Deve retornar erro quando a área de vegetação for zero", () => {
    ...
    const erro = fazendaService.ValidarDadosFazenda(fazendaAreaVegetacaoZero);
    expect(erro).toBe("Precisa informar a área de vegetação");
  });

  it("Deve retornar erro quando a soma da área agricultável e área de vegetação for maior que a área total", () => {
    ...
    const error = fazendaService.ValidarDadosFazenda(fazendaAreaInvalida);
    expect(error).toBe(
      `A soma da área de vegetação e a área agricultável da fazenda '${fazendaAreaInvalida.Nome}' não pode ser maior que a área total`
    );
  });
});
...
describe("Validação do serviço de fazenda", () => {
  ...
  describe("CriarFazendas", () => {
    ...
    it("ira gera um execeção quando erro", async () => {
      ...
      await expect(
        fazendaService.CriarFazendas(idProdutor, FazendaASerCriadamock)
      ).rejects.toThrow();
    });
  });
  ...
  describe("RemoverFazendaNaoInclusas", () => {
    it("deve remover fazendas que não estão na lista", async () => {
      const fazendasExistentes = [
        { ...mockFazenda, Id: 2 },
        { ...mockFazenda, Id: 1 },
      ] as FazendaModel[];

      fazendaRepositorioMock.BuscarFazendasPorIDProdutor.mockResolvedValue(
        fazendasExistentes
      );
  ...
  describe("ObterCodigosFazendaPorProdutor", () => {
    it("deve retornar os IDs das fazendas do produtor", async () => {
      fazendaRepositorioMock.ObterCodigosFazendasPorProdutor.mockResolvedValue([
        1, 2, 3,
      ]);
  ...
  describe("RemoverFazendasPorIdProdutor", () => {
    it("deve remover todas as fazendas de um produtor", async () => {
      ...
      expect(
        fazendaRepositorioMock.RemoverFazendaPorIdProdutor
